/****************************************************************************************
 *  Kamaz-Leveler - FreeRTOS + OTA (версия 8.6.0)
...
/* ====================  ПАРАМЕТРЫ ==================== */
constexpr const char *VERSION = "V 8.6.0 FreeRTOS OTA";
...
void checkAndAdjustMasterPressure() {
  ...
  uint32_t now = millis();
  if (now - lastMasterPressureCheckTime < MASTER_PRESSURE_CHECK_INTERVAL_MS) {
    return;
  }
...
void displayOTAScreen() {
  ...
  if (!initialized) {
    ...
    uiHeader("РЕЖИМ OTA", theme::ACCENT);
    ...
    uiHintBar("МЕНЮ — ВЫХОД", theme::TEXT_DIM);
...
void displayErrorScreen() {
  ...
  if (needRefresh) {
    ...
    ui.setFont(UiFont::Tiny);
    ui.setColors(theme::TEXT, COLOR_ERROR);
    ui.box(theme::MARGIN, 212, 150, 18, "КН3 — СБРОС", UiHAlign::Left, UiVAlign::Middle, true);
    ui.box(theme::SCREEN_W - theme::MARGIN - 150, 212, 150, 18, "МЕНЮ — НАЗАД", UiHAlign::Right, UiVAlign::Middle, true);
...
void displayMainScreen() {
  ...
  if (abs(localAngleX - lastState.angleX) > 0.05f || abs(localAngleY - lastState.angleY) > 0.05f ||
      abs(localTemp - lastState.temp) > 0.5f || abs(localMasterPressure - lastState.masterPressure) > 0.05f ||
...
void updateTestDisplay() {
  ...
  if (!initialized && currentTestStep != TestStep::COMPLETED && currentTestStep != TestStep::IDLE) {
    ...
    ui.box(theme::MARGIN, HDR_Y, theme::SCREEN_W - 2 * theme::MARGIN, HDR_H, "ТЕСТ КЛАПАНОВ",
           UiHAlign::Center, UiVAlign::Middle, false);
    uiHintBar("КН4 — остановить тест", theme::WARN);
    initialized = true;
  }
...
static void renderTestResults() {
  ...
  ui.setFont(UiFont::SmallB);
  ui.setColors(theme::WARN, theme::BG);
  ui.box(0, theme::SCREEN_H - 22, theme::SCREEN_W, 20, "МЕНЮ — ВЫХОД",
         UiHAlign::Center, UiVAlign::Middle, false);
}
...
void initGEM() {
    ...
    static GEMItem itemOTA("Режим OTA", []() {
        menuVisible = false;
        displayDirty = true;
        startOTAMode();
    });
    ...
    static GEMItem itemGitHubUpdate("Проверить GitHub", []() {
        ...
        Logger::log(Logger::INFO, "GH-OTA", "Запрос отправлен в OTATask");
        menuVisible = false;
        displayDirty = true;
    });
