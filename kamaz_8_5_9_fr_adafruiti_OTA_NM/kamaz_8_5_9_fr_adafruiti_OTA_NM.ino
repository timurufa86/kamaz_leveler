/****************************************************************************************
 *  Kamaz-Leveler - FreeRTOS + OTA (версия 8.5.9)
...
/* ====================  ПАРАМЕТРЫ ==================== */
constexpr const char *VERSION = "V 8.5.9 FreeRTOS OTA";
    ...
    if (menuVisible) {
      ...
      if (millis() - lastMenuButtonTime >= MENU_DEBOUNCE_MS) {
        ...
        if (button0.click()) {  // Кн1 - ВВЕРХ
          gem.registerKeyPress(GEM_KEY_UP);   // GEM сам перерисовывает меню
          lastMenuButtonTime = millis();
          Serial.println("[MENU] UP");
        }
        if (button1.click()) {  // Кн2 - ВНИЗ
          gem.registerKeyPress(GEM_KEY_DOWN); // GEM сам перерисовывает меню
          lastMenuButtonTime = millis();
          Serial.println("[MENU] DOWN");
        }
        if (button2.click()) {  // Кн3 - ВЫБОР/OK
          gem.registerKeyPress(GEM_KEY_OK);   // GEM сам перерисовывает меню
          lastMenuButtonTime = millis();
          Serial.println("[MENU] SELECT");
        }
        if (button3.click()) {  // Кн4 - НАЗАД/CANCEL
          gem.registerKeyPress(GEM_KEY_CANCEL); // GEM сам перерисовывает меню
          lastMenuButtonTime = millis();
