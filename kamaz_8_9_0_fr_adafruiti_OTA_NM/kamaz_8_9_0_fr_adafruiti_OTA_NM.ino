...
/* ====================  ПАРАМЕТРЫ ==================== */
constexpr const char *VERSION = "V 8.9.0 FreeRTOS OTA";
...
static void displayMovementScreen() {
  ...
  /* --- статус-строка: иконка режима, пульс, таймер движения, T и МП --- */
  drawIconL(4, 0, iconMovingL, theme::WARN);
  ui.setFont(UiFont::SmallB);
  ui.setColors(theme::WARN, theme::BG);
  ui.box(28, 0, 92, theme::STATUS_H, "ДВИЖЕНИЕ", UiHAlign::Left, UiVAlign::Middle, true);

  if (((now / 500U) % 2U) == 0U) tft.fillCircle(124, (int16_t)(theme::STATUS_H / 2), 3, theme::WARN);
  else tft.fillCircle(124, (int16_t)(theme::STATUS_H / 2), 3, theme::BG);

  ui.setFont(UiFont::Small);
  ui.setColors(theme::TEXT, theme::BG);
  ui.boxf(134, 0, 66, theme::STATUS_H, UiHAlign::Left, UiVAlign::Middle, true, "%lu:%02lu",
          (unsigned long)(d.inMotionMs / 60000UL), (unsigned long)((d.inMotionMs / 1000UL) % 60UL));
  ui.setColors(theme::ACCENT, theme::BG);
  ui.boxf(200, 0, theme::SCREEN_W - 206, theme::STATUS_H, UiHAlign::Right, UiVAlign::Middle, true,
          "%.0f C  МП %.1f", mvTempDisp, mvMasterDisp);

  /* --- блок целей: передние / задние (факт, цель, Δ по допуску) --- */
  const float dF = d.frontAvg - d.targetFront;
  const float dR = d.rearAvg - d.targetRear;
  const uint16_t colF = (fabsf(dF) <= d.tol * 0.5f) ? theme::OK : ((fabsf(dF) <= d.tol) ? theme::WARN : theme::ERR);
  const uint16_t colR = (fabsf(dR) <= d.tol * 0.5f) ? theme::OK : ((fabsf(dR) <= d.tol) ? theme::WARN : theme::ERR);
  const uint16_t colRealF = (d.frontAvg > pScale) ? theme::ERR : ((d.frontAvg < pMin) ? theme::WARN : colF);
  const uint16_t colRealR = (d.rearAvg > pScale) ? theme::ERR : ((d.rearAvg < pMin) ? theme::WARN : colR);
  ...
  ui.boxf(MV_X + 56, 25, 76, 19, UiHAlign::Left, UiVAlign::Middle, true, "%.2f", mvFrontDisp);
  ui.boxf(MV_X + 56, 57, 76, 19, UiHAlign::Left, UiVAlign::Middle, true, "%.2f", mvRearDisp);

  ui.setFont(UiFont::Small);
  ui.setColors(theme::TEXT_DIM, theme::PANEL);
  ui.boxf(MV_X + 134, 27, 64, 16, UiHAlign::Left, UiVAlign::Middle, true, "цель %.2f", d.targetFront);
  ui.boxf(MV_X + 134, 59, 64, 16, UiHAlign::Left, UiVAlign::Middle, true, "цель %.2f", d.targetRear);

  ui.setFont(UiFont::SmallB);
  ui.setColors(colF, theme::PANEL);
  ui.boxf(theme::SCREEN_W - MV_X - 76, 27, 72, 16, UiHAlign::Right, UiVAlign::Middle, true, "%+.2f", dF);
  ui.setColors(colR, theme::PANEL);
  ui.boxf(theme::SCREEN_W - MV_X - 76, 59, 72, 16, UiHAlign::Right, UiVAlign::Middle, true, "%+.2f", dR);

  mvDrawBar(MV_X + 4, 46, theme::SCREEN_W - 2 * MV_X - 8, 6, mvFrontDisp, pScale, d.targetFront, colRealF);
  mvDrawBar(MV_X + 4, 78, theme::SCREEN_W - 2 * MV_X - 8, 6, mvRearDisp, pScale, d.targetRear, colRealR);

  if (d.activeFront)
    mvDrawFlow(MV_X + 4, 46, theme::SCREEN_W - 2 * MV_X - 8, 6, (d.dirFront >= 0) ? theme::OK : theme::WARN, now);
  if (d.activeRear)
    mvDrawFlow(MV_X + 4, 78, theme::SCREEN_W - 2 * MV_X - 8, 6, (d.dirRear >= 0) ? theme::OK : theme::WARN, now);

  /* --- карточки подушек: значение, метка цели, Δ --- */
  for (uint8_t i = 0; i < PAD_COUNT; i++) {
    const int16_t cx = (i % 2 == 0) ? MV_X : (int16_t)(theme::SCREEN_W / 2 + 1);
    const int16_t cy = (i < 2) ? 94 : 136;
    const float target = (i < 2) ? d.targetFront : d.targetRear;
    const float diff = d.pad[i] - target;
    const uint16_t col = (fabsf(diff) <= d.tol * 0.5f) ? theme::OK : ((fabsf(diff) <= d.tol) ? theme::WARN : theme::ERR);
    const uint16_t barCol = (d.pad[i] > pScale) ? theme::ERR : ((d.pad[i] < pMin) ? theme::WARN : theme::OK);
    ...
    ui.setFont(UiFont::Med);
    ui.setColors(theme::TEXT, theme::PANEL);
    ui.boxf(cx + 42, cy + 1, 66, 19, UiHAlign::Left, UiVAlign::Middle, true, "%.2f", mvPadDisp[i]);

    ui.setFont(UiFont::Small);
    ui.setColors(col, theme::PANEL);
    ui.boxf(cx + MV_CARD_W - 72, cy + 3, 66, 15, UiHAlign::Right, UiVAlign::Middle, true, "%+.2f", diff);
    ...
    if (isActive) {
      ...
      mvDrawFlow(cx + 6, cy + MV_CARD_H - 9, MV_CARD_W - 12, 5, (dir >= 0) ? theme::OK : theme::WARN, now);
    }
  }

  /* --- тренд давлений за 30 с --- */
  ui.setFont(UiFont::Tiny);
  ui.setColors(theme::TEXT_DIM, theme::BG);
  ui.box(MV_X, 178, 160, 12, "ТРЕНД 30 с:  ПЕРЕД  ЗАД", UiHAlign::Left, UiVAlign::Middle, true);
  mvDrawTrend(MV_X, 192, 150, 42, d.targetFront, d.targetRear, pScale);

  /* --- правая колонка: таймеры и активные клапаны --- */
  ui.setFont(UiFont::Tiny);
  if (d.settleLeftMs > 0) {
    ui.setColors(theme::WARN, theme::BG);
    ui.boxf(164, 178, 148, 14, UiHAlign::Left, UiVAlign::Middle, true, "СТАБ %lu с  (возврат)",
            (unsigned long)(d.settleLeftMs / 1000UL));
    ...
    ui.setColors(theme::TEXT_DIM, theme::BG);
    ui.box(164, 178, 148, 14, "СТАБ — (не начата)", UiHAlign::Left, UiVAlign::Middle, true);
    tft.fillRect(164, 192, 148, 4, theme::BG);
  }

  ui.setFont(UiFont::Small);
  ui.setColors(theme::ACCENT, theme::BG);
  ui.boxf(164, 198, 148, 14, UiHAlign::Left, UiVAlign::Middle, true, "ПРОВЕРКА %lu:%02lu",
          (unsigned long)(d.nextCheckMs / 60000UL), (unsigned long)((d.nextCheckMs / 1000UL) % 60UL));

  ui.setColors(theme::TEXT_DIM, theme::BG);
  if (d.lastAdjFrontMs == 0 && d.lastAdjRearMs == 0) {
    ui.box(164, 212, 148, 14, "КОРРЕКТИРОВОК НЕ БЫЛО", UiHAlign::Left, UiVAlign::Middle, true);
  } else {
    ui.boxf(164, 212, 148, 14, UiHAlign::Left, UiVAlign::Middle, true, "КОРР П:%luс З:%luс",
            (unsigned long)(d.lastAdjFrontMs / 1000UL), (unsigned long)(d.lastAdjRearMs / 1000UL));
  }

  ui.setColors((d.activeFront || d.activeRear) ? theme::OK : theme::TEXT_DIM, theme::BG);
  if (!d.activeFront && !d.activeRear) {
    ui.box(164, 226, 148, 14, "АКТИВНО: —", UiHAlign::Left, UiVAlign::Middle, true);
  } else if (d.activeFront && d.activeRear) {
    ui.box(164, 226, 148, 14, "АКТИВНО: ПЕРЕД+ЗАД", UiHAlign::Left, UiVAlign::Middle, true);
  } else if (d.activeFront) {
    ui.boxf(164, 226, 148, 14, UiHAlign::Left, UiVAlign::Middle, true, "АКТИВНО: %s ПЕРЕД",
            (d.dirFront >= 0) ? "НАКАЧ" : "СБРОС");
  } else {
    ui.boxf(164, 226, 148, 14, UiHAlign::Left, UiVAlign::Middle, true, "АКТИВНО: %s ЗАД",
            (d.dirRear >= 0) ? "НАКАЧ" : "СБРОС");
  }
}
...
void displayMainScreen() {
  mvScreenWasActive = false;  // 8.9.0: вернулись с экрана ДВИЖЕНИЯ — перерисовать каркас
  ...
  for (;;) {
    ...
    if (xSemaphoreTake(xDisplayMutex, pdMS_TO_TICKS(500)) == pdTRUE) {
      ...
      if (ErrorHandler::hasActiveErrors()) {
        ...
        // ============================================================
        if (currentSystemMode == SystemMode::MOVEMENT && currentState == SystemState::RUNNING &&
