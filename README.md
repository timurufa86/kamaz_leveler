...
- Базовая ветка разработки: **`8.8.0`**
- Скетч: `kamaz_8_8_0_fr_adafruiti_OTA_NM/`
...
arduino-cli compile --config-file arduino-cli.yaml `
  ...
  kamaz_8_8_0_fr_adafruiti_OTA_NM
```
...
разделов** (`kamaz_8_8_0_fr_adafruiti_OTA_NM/partitions.csv`, включается
...
Скетч занимает ~1.35 МБ (1 411 974 Б, 8 % от 16 МБ), то есть в каждом app-слоте
...
- SemVer: см. `.cursor/rules/semantic-versioning.mdc` (стиль папок/веток `8.8.0`).
- OTA-релиз: см. `.cursor/rules/github-ota-release.mdc`
  (ассеты `kamaz_leveler.bin` + `kamaz_leveler.bin.sha256`, тег `v8.8.0`).
